...
if not already_loaded_dataset_and_set_up_variables:
    ...
    cooccurrence = CoOccurrence_Christian()
    ...
    # todo for now i set up a second cooccurrence in case they are different
    cooccurrence2 = CoOccurrence()
    cooccurrence2.generate_occurrences(dtype_co_occurrence=np.uint32,
                                       csv_path=".")  # network viz master is the cwd for some reason
    cooccurrence2.generate_graph()
    cooccurrence2.generate_partition()
    layout = nx.drawing.layout.kamada_kawai_layout(G=cooccurrence2.tag_graph)
        ...
        dbc.Col(
            [
                dcc.Graph(
                    ...
                    figure=make_network_fig(cooccurrence2.tag_graph, cooccurrence2, [], layout, None)
                ),
                dcc.Markdown(
...
def update_tags_chart(input_countries, input_categories, date_string_from_hidden_rangeslider_div,
...
def redraw_fig(selectedData, searchData, clickData):
    ...
    if trigger == "search-box" or trigger == "tags-chart":  # todo bugs when not typing fast enough
        ...
        if selectedData:
            selectedPoints = [selectedData["points"][i]["customdata"] for i in range(len(selectedData["points"]))]
    return make_network_fig(cooccurrence2.tag_graph, cooccurrence2, selectedPoints, layout, None)
